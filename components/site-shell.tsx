  ...
  return (
    <div className={cn("min-h-screen bg-[#f4f7fb] text-foreground", className)}>
          ...
          <nav className="flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
            {publicLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="rounded-full px-4 py-2.5 transition hover:bg-muted hover:text-foreground"
              >
                {link.label}
              </Link>
            ))}
      ...
      <footer className="border-t border-border/70 bg-white/80">
        <div className="mx-auto flex w-full max-w-7xl flex-col gap-4 px-4 py-6 text-sm text-muted-foreground md:flex-row md:items-center md:justify-between md:px-6 md:py-7 lg:px-8">
